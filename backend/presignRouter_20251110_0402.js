...
// ★ 추가 1: 외부(브라우저/호스트)에서 접근 가능한 공개 URL
// 예) 로컬:  http://127.0.0.1:9000
// 예) 운영:  https://uconcreative.ddns.net:9000  (환경에 맞게 설정)
const PUBLIC_MINIO_URL = process.env.MINIO_PUBLIC_URL || "";

const minio = new MinioClient({
  endPoint: process.env.MINIO_HOST || "minio", // 컨테이너 내부 접근용
...
router.post("/presign/upload", auth, async (req, res) => {
  try {
    ...
    let url = await minio.presignedPutObject(BUCKET, objectName, expirySec, {
      "Content-Type": contentType || "application/octet-stream",
    });

    // ★ 추가 2: presign URL의 호스트(minio:9000)를 외부 공개 URL로 치환
    if ( PUBLIC_MINIO_URL ) {
      try {
        const u = new URL(url);           // presign 결과 (ex: http://minio:9000/...)
        const p = new URL(PUBLIC_MINIO_URL); // 공개용 (ex: http://127.0.0.1:9000)
        u.protocol = p.protocol;          // http/https
        u.host = p.host;                  // host:port
        url = u.toString();
      } catch {
        // 치환 실패 시 원본 url 그대로 사용
      }
    }
